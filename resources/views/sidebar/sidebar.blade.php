<div x-data="{ showData: false, showReport: false }" class="flex flex-col w-64 h-screen bg-white text-[#666]">
    ...
    <nav class="flex-1 py-3 overflow-y-auto overflow-x-hidden">
        <a href="#" class="block w-full px-6 py-3 text-base font-semibold text-[#666] hover:text-[#631111] hover:ml-1 flex items-center transition-all duration-200">
            <img src="{{ asset('assets/icons/monitor.svg') }}" alt="Dashboard Icon" class="w-6 h-6 mr-2">
            Dashboard
        </a>
        <button @click="showData = !showData" class="block w-full px-6 py-3 text-base font-semibold text-left text-[#666] hover:text-[#631111] hover:ml-1 flex items-center transition-all duration-200">
            <img src="{{ asset('assets/icons/data.svg') }}" alt="Data Management Icon" class="w-6 h-6 mr-2">
            Data Management
            <i class="ml-auto transition-transform duration-200 fas fa-chevron-right " :class="{'transform rotate-90': showData, 'transform rotate-0': !showData}"></i>
        </button>
        <div x-show="showData" x-transition:enter="transition-all ease duration-700" 
        ...
        <button @click="showReport = !showReport" class="block w-full px-6 py-3 text-base font-semibold text-left text-[#666] hover:text-[#631111] hover:ml-1 flex items-center transition-all duration-200">
            <img src="{{ asset('assets/icons/report.svg') }}" alt="Reports Icon" class="w-6 h-6 mr-2">
            Reports
            <i class="ml-auto transition-transform duration-200 fas fa-chevron-right " :class="{'transform rotate-90': showReport, 'transform rotate-0': !showReport}"></i>
        </button>
        <div x-show="showReport" x-transition:enter="transition-all ease duration-700" 
             x-transition:enter-start="max-h-0 opacity-0" 
             x-transition:enter-end="max-h-screen opacity-100" 
             x-transition:leave="transition-all ease duration-500" 
             x-transition:leave-start="max-h-screen opacity-100" 
